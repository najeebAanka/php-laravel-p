    ...
    <script>
        ...
        $(function() {
            ...
            function fetchAllOrders() {
                $.ajax({
                    url: server + "orders/fetchall?page=" + current_page,
                    dataType: 'json',
                    method: 'get',
                    success: function(r) {
                        let response = r.data;
                        ...
                        for (var i = 0; i < response.length; i++) {
                            str += "<tr><td>" + response[i].id + "</td><td>" + response[i].name +
                                ...
                                "'  data-status='" + response[i].status +
                                "'  data-customer_name='" + response[i].name +
                                "'  class='btn btn-secondary btn-sm mt-1 view-order-btn' style='margin-left: 4px;' href='" +  'single-order/' + response[i].id + "'>View <i class=\"bi-eye \"></i></a><button data-id='" + response[i]
                                .id +
                                "'  data-customer_name='" + response[i].name +
                        ...
                        $('.edit-order-btn').click(function() {
                            ...
                            $('#edit-order-title').html("Edit " + $(this).attr('data-customer_name') + " order");
                        ...
                        $('.del-order-btn').click(function() {
                            ...
                            if (confirm("Are you sure to remove " + $(this).attr('data-customer_name') +  " order?")) {

                                $(this).html("Deleting...");
                                $(this).prop("disabled", true);

                                $.ajax({
                                    url: server + "orders/delete",
                                    method: 'delete',
                                    data: {
                                        id: id,
                                        _token: csrf
