...
<?php 
$order  = App\Models\Order::find(Route::input('id'));
    ...
    <script>
        ...
        $(function() {
            ...
            $("#edit_order_form").submit(function(e) {
                ...
                $.ajax({
                    url: server + "single-order/update",
                    method: 'post',
                    data: fd,
                    cache: false,
                    contentType: false,
            ...
            function fetchAllOrders() {
                $.ajax({
                    url: server + "single-order/fetchall"+"/"+<?php echo $o->id; ?>+"?page=" + current_page,
                    dataType: 'json',
                    method: 'get',
                    success: function(r) {
                        let response = r.data;
                        ...
                        for (var i = 0; i < response.length; i++) {
                            str += "<tr><td>" + response[i].id + "</td><td>" + response[i].service_name +
                                ...
                                "</td><td style='text-align : center'><button data-order_item_id='" + response[i]
                                .order_item_id +
                                ...
                                "'  class='btn btn-success btn-sm mt-1 edit-order-btn'>Edit <i class=\"bi-pencil-square \"></i></button><button data-id='" + response[i]
                                .id +
                                "'  data-service_id='" + response[i].service_id +
                        ...
                        $('.del-order-btn').click(function() {
                            ...
                            if (confirm("Are you sure to remove this order item?")) {
                                ...
                                $.ajax({
                                    url: server + "single-order/delete",
                                    method: 'delete',
                                    data: {
                                        id: id,
            ...
            function fetchAllOrdersPagesOnly() {
                $.ajax({
                    url: server + "single-order/fetchall"+"/"+<?php echo $o->id; ?>+"?page=" + current_page,                    dataType: 'json',
                    method: 'get',
                    success: function(r) {
                        let response = r.data;
                        console.log(response.length)
